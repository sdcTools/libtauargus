...
void CTauArgCtrl::FireUpdateProgress (short Perc)
{
...
STDMETHODIMP CTauArgCtrl::SetHierarchicalDigits(long VarIndex,
												...
												VARIANT_BOOL *pVal)
{
  ...
  *CodeString = m_var[v].GetCode(CodeIndex).AllocSysString();
...
STDMETHODIMP CTauArgCtrl::SetInFileInfo(VARIANT_BOOL IsFixedFormat, BSTR Seperator)
{
	if (IsFixedFormat) {
		...
		InFileIsFixedFormat = false;
	}

	InFileSeperator = Seperator;
	return S_OK;
}
...
int CTauArgCtrl::ReadMicroRecord(FILE *fd, UCHAR *str)
{
	int length = 0;
...
int CTauArgCtrl::DoMicroRecord(UCHAR *str, int *varindex)
{
	int i, bp, ap;
...
int CTauArgCtrl::BinSearchStringArray(CStringArray &s, CString x, int nMissing, bool &IsMissing)
{
...
/*
void CTauArgCtrl::QuickSortMaxScore(double &doub, int &intg, int first, int last)
{
...
*/
// sort strings
void CTauArgCtrl::QuickSortStringArray(CStringArray &s, int first, int last)
{
	int i, j;
	CString mid, temp;
...
BOOL CTauArgCtrl::ConvertNumeric(char *code, double &d)
...
BOOL CTauArgCtrl::FillInTable(long Index, CString *sCodes, double Cost,
...
void CTauArgCtrl::FillTables(UCHAR *str)
{
	int i, j;
	char code[MAXCODEWIDTH];
	bool IsMissing;
...
void CTauArgCtrl::AddTableCells(CTable &t, CDataCell AddCell, int niv, long cellindex)
{
...
void CTauArgCtrl::AddTableCell(CTable &t, CDataCell AddCell, long cellindex)
{
...
void CTauArgCtrl::AddTableToTableCell(CTable &tabfrom, CTable &tabto,
												  long ifrom, long ito)
{
	CDataCell *dc1, *dc2;
...
void CTauArgCtrl::MergeLastTempShadow()
{
	int i,j,k,l;
	CTable *tab;
	CDataCell *dc;
...
void CTauArgCtrl::InitializeHoldingTables()
{
	int i;
	CTable *tab;
	for (i=0; i<m_ntab; i++){
...
BOOL CTauArgCtrl::ParseRecodeString(long VarIndex, LPCTSTR RecodeString, long FAR* ErrorType, long FAR* ErrorLine, long FAR* ErrorPos, int Phase)
{
	char *p;
...
BOOL CTauArgCtrl::ParseRecodeStringLine(long VarIndex, LPCTSTR str, long FAR* ErrorType, long FAR* ErrorPos, int Phase)
{
...
int CTauArgCtrl::ReadWord(LPCTSTR str, char* CodeFrom, char *CodeTo, char EndCode, int& fromto, int& pos)
{
	int i = 0, j, pass = 0;
	char *p;
...
void CTauArgCtrl::AddSpacesBefore(char *str, int len)
{
	int lstr = strlen(str);
...
void CTauArgCtrl::AddSpacesBefore(CString& str, int len)
{
...
int CTauArgCtrl::SetCode2Recode(int VarIndex, char *DestCode, char *SrcCode1, char *SrcCode2, int fromto)
{
...
BOOL CTauArgCtrl::ComputeRecodeTables()
{
	int i, d, nRecodes = 0;
...
// returns index
int CTauArgCtrl::AddRecode(int varnr, const char *newcode)
{
	int i, n;
...
int CTauArgCtrl::MakeRecodelistEqualWidth(int VarIndex, LPCTSTR Missing1, LPCTSTR Missing2)
{
...
BOOL CTauArgCtrl::ComputeRecodeTable(CTable& srctab, CTable& dsttab)
{
	int d;
...
void CTauArgCtrl::ComputeRecodeTableCells(
...
void CTauArgCtrl::ComputeCellStatuses(CTable &tab)
{
	int c,j,St;
	CDataCell *dc;
	long *DimNr;
...
void CTauArgCtrl::SetProtectionLevels(CTable &tab)
{
	int c;
	CDataCell *dc;
...
void CTauArgCtrl::SetTableHasRecode()
{
	int t, v;
...
void CTauArgCtrl::WriteCSVTable(FILE *fd, CTable *tab, long *DimSequence, long *Dims, int niv, char ValueSep, long RespType)
{
	int i, n;
...
// show column labels
void CTauArgCtrl::WriteCSVColumnLabels(FILE *fd, CTable *tab, long dim, char ValueSep)
{
...
void CTauArgCtrl::WriteCSVLabel(FILE *fd, CTable *tab, long dim, int code)
{
	CString s;
...
// write cells
void CTauArgCtrl::WriteCSVCell(FILE *fd, CTable *tab, long *Dim, bool ShowUnsafe, int SBSCode, long RespType)
{
	CDataCell *dc;
...
void CTauArgCtrl::WriteFirstLine(FILE *fd, LPCTSTR FirstLine)
{
	int LenFirstLine;
...
// write SBSStaart
void CTauArgCtrl::WriteSBSStaart(FILE *fd, CTable *tab, long *Dim, char ValueSep, long SBSCode)
{
	CDataCell *dc;
	dc = tab->GetCell(Dim);
	int f1, f2;
...
// write cell
void CTauArgCtrl::WriteCellRecord(FILE *fd, CTable *tab,
...
void CTauArgCtrl::WriteCellDimCell(FILE *fd, CTable *tab,
...
CTable * CTauArgCtrl::GetTable(int t)
{
  ASSERT( t >= 0 && t < m_ntab);
...
// only for debug
void CTauArgCtrl::ShowCodeLists()  // in output pane
{ int i, j;
  CString s;
  for (i = 0; i < m_nvar; i++) {
    CVariable *v = &(m_var[i]);
...
// only for debug
BOOL CTauArgCtrl::ShowTable(const char *fname, CTable& tab)
{ FILE *fd;
  CDataCell *dc;
...
// only for debug
int CTauArgCtrl::ShowTableLayerCell(char *str, double val, int ndec)
{ int c;
...
int CTauArgCtrl::ShowTableLayerCell(char *str, long val)
{ int c;
...
// only for debug
void CTauArgCtrl::ShowTableLayer(FILE *fd, int var1, int var2, int cellnr, CTable& tab)
...
bool CTauArgCtrl::IsTable (CTable *tab)
{
	long DimNr[MAXDIM];
	long d;
...
void CTauArgCtrl::AdjustTable(CTable *tab)
{
	long DimNr[MAXDIM];
	long d;
	for (d = 0; d < tab->nDim; d++) {
...
void CTauArgCtrl::TestTable(CTable *tab, long TargetDim,
...
int CTauArgCtrl::GetChildren(CVariable &var, int CodeIndex, CUIntArray &Children)
{
...
void CTauArgCtrl::AdjustNonBasalCells(CTable *tab, long TargetDim, long *DimNr, long niv)
{
...
bool CTauArgCtrl::ReadVariableFreeFormat(UCHAR *Str, long VarIndex, CString *VarCode)
{
...
long CTauArgCtrl::MaxDiepteVanSpanVariablen(CTable *tab)
{
...
// a real interface.
bool CTauArgCtrl::TestSubCodeList()
{
	CTable *tab;
	CVariable *var;
	long i;
...
long CTauArgCtrl::NumberOfSubTables(long IndexTable)
{
	CTable *tab;
	CVariable *var;
	long i;
...
bool CTauArgCtrl::SubTableTupleForSubTable(long TableIndex, long SubTableIndex,
													 long *SubCodeIndices)
{
	CTable *tab;
	CVariable *var;
...
long CTauArgCtrl::WriteCellInTempFile(long UnsafeCellNum, long TableIndex, long CellNum, FILE *fdtemp, double MaxScale) //long *CellDimension)
{
	long i,j,k,l;
...
bool CTauArgCtrl::ArrayCompare (long *arr1, long *arr2, long nDim)
{
	bool equal = true;
	long i;
	for (i = 0; i<nDim; i++)	{
...
long CTauArgCtrl::FindNumberOfElementsInSubTable(long *SubTableTuple, long TableIndex)
{
	long i;
	CTable *tab;
	CVariable *var;
...
// Index of this cell
bool CTauArgCtrl::FindCellIndexForSubTable(long *TableCellIndex,
...
long CTauArgCtrl::SubTableForCellDimension(long TableIndex, long *CellDimension, long * SubTableTupleIndex)
{
	long i,j;
	long SubTableIndex = -1;
	long *SubTableTuple;
...
long CTauArgCtrl::FindSubTableForCell(long CellIndex, long TableIndex, long *SubTableTupleIndex)
{
	long *dimnr;
	CTable *tab;
	tab = &(m_tab[TableIndex]);
...
bool CTauArgCtrl::WriteAllSubTablesInAMPL(FILE *fd, long tabind)
{
	long i, j, k, cellnr;
	CTable *tab;
	CVariable *var1, *var;
...
// This is is not generalized but can be with some minor changes
bool CTauArgCtrl::WriteTableSequenceHierarchyInAMPL(FILE *fd, long tabind,
																	 long varind)
{
	CTable *tab;
	CVariable *var;
...
bool CTauArgCtrl::WriteHierTableInAMPL(FILE *fd, long tabind, CString TempDir, double MaxScale)
{
	long nsubtab;
	CVariable *var0;
	CVariable *var1;
...
bool CTauArgCtrl::testampl(long ind)
{
...
//DEL STDMETHODIMP CTauArgCtrl::Anco(long longvar1, long *longvar2, VARIANT_BOOL *pVal)
//DEL {
//DEL 	bool bla;
//DEL 	if (bla)	{
